        ...
        public async Task<IActionResult> GetByUser(int userId)
        {
            var data = await _context.Bookings
                ...
                .Select(b => new
                {
                    ...
                    b.TimeSlotId,
                    time = b.TimeSlot.StartTime.ToString(@"hh\:mm") + " - " + b.TimeSlot.EndTime.ToString(@"hh\:mm"),
                    paymentMethod = _context.Payments
                        .Where(p => p.BookingId == b.BookingId)
                        .Select(p => p.PaymentType)
                        .FirstOrDefault() ?? "Simulated",
                    paidAmount = _context.Payments
                        .Where(p => p.BookingId == b.BookingId && p.PaymentStatus == "Success")
                        .Select(p => p.Amount)
                        .FirstOrDefault()
                })
                .ToListAsync();
